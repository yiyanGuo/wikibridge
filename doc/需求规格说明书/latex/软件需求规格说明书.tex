...
\begin{tikzpicture}[
  ...
  usecase/.style={draw=whu, thick, ellipse, fill=white, align=center, minimum width=3.15cm, minimum height=0.78cm},
...
\node[actor] (B) at (-5.7,2.35) {B 端\\知识拥有者};
\node[actor] (C) at (-5.7,-2.05) {C 端\\知识消费者};
\node[actor] (S) at (5.7,2.1) {S 端\\管理员};
\node[actor] (D) at (5.7,-2.35) {项目\\开发者};

\node[usecase] (uc1) at (-1.9,2.75) {构建本地\\知识库};
\node[usecase] (uc2) at (-1.9,1.55) {发布\\LLM-Wiki API};
\node[usecase] (uc3) at (1.75,2.1) {管理\\BearFRP 代理};
\node[usecase] (uc4) at (-1.9,0.05) {添加远程\\知识库};
\node[usecase] (uc5) at (-1.9,-1.2) {启动本地\\OpenCode};
\node[usecase] (uc6) at (1.75,-0.65) {注册\\llm-wiki MCP};
\node[usecase] (uc7) at (1.75,-1.95) {对话式读取/\\搜索/总结};
\node[usecase] (uc8) at (1.75,0.65) {维护部署、\\联调与测试};
\node[usecase] (uc9) at (1.75,-3.2) {执行安全\\边界检查};
...
\draw[link] (D) -- (uc8);
\draw[link] (D) -- (uc9);
\draw[incl] (uc2) -- node[edgelabel,above]{include} (uc3);
\draw[incl] (uc7) -- node[edgelabel,below]{include} (uc4);
\draw[incl] (uc7) -- node[edgelabel,right]{include} (uc6);
\draw[incl] (uc9) -- node[edgelabel,above]{extend} (uc5);
\draw[incl] (uc9) -- node[edgelabel,right]{extend} (uc6);
\end{tikzpicture}}
