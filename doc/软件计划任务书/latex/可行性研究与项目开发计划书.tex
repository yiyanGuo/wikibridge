...
\begin{tikzpicture}[
  font=\small,
  actor/.style={draw=whu, thick, rounded corners=3pt, fill=softgray, align=center, minimum width=2.4cm, minimum height=0.85cm},
  usecase/.style={draw=whu, thick, ellipse, fill=white, align=center, minimum width=2.9cm, minimum height=0.78cm},
...
\node[actor] (B) at (-5.2,2.2) {B 端\\知识拥有者};
\node[actor] (C) at (-5.2,-2.1) {C 端\\知识消费者};
\node[actor] (S) at (5.2,2.2) {S 端\\管理员};
\node[actor] (D) at (5.2,-2.1) {项目\\开发者};

\node[usecase] (uc1) at (-1.7,2.55) {构建本地\\知识库};
\node[usecase] (uc2) at (-1.6,1.35) {发布\\LLM-Wiki API};
\node[usecase] (uc3) at (1.55,2.05) {管理\\BearFRP 代理};
\node[usecase] (uc4) at (-1.7,-0.1) {添加远程\\知识库};
\node[usecase] (uc5) at (-1.7,-1.3) {启动本地\\OpenCode};
\node[usecase] (uc6) at (1.55,-0.75) {注册\\llm-wiki MCP};
\node[usecase] (uc7) at (1.55,-2.0) {对话式读取/\\搜索知识库};
\node[usecase] (uc8) at (1.55,0.55) {维护部署\\与 CI};
\node[usecase] (uc9) at (1.55,-3.25) {执行联调\\与测试};
...
\draw[link] (D) -- (uc8);
\draw[link] (D) -- (uc9);
\draw[incl] (uc2) -- node[label,above]{include} (uc3);
\draw[incl] (uc7) -- node[label,right]{include} (uc6);
\draw[incl] (uc7) -- node[label,below]{include} (uc4);
\end{tikzpicture}}
...
\begin{tikzpicture}[
  font=\scriptsize,
  root/.style={draw=whu, thick, rounded corners=4pt, fill=softblue, align=center, minimum width=3.4cm, minimum height=0.75cm},
  phase/.style={draw=whu, thick, rounded corners=3pt, fill=white, align=center, minimum width=2.55cm, minimum height=0.72cm},
  work/.style={draw=linegray, rounded corners=2pt, fill=softgray, align=center, minimum width=2.25cm, minimum height=0.55cm},
  flow/.style={draw=whu, thick}
]
\node[root] (root) at (0,0) {WikiBridge 项目开发};
\foreach \i/\name in {-6/需求与架构,-4/知识库模块,-2/发布控制面,0/桌面端集成,2/OpenCode + MCP,4/部署与 CI,6/测试与文档} {
  \node[phase] (p\i) at (\i,-1.25) {\name};
  \draw[flow] (root.south) |- (p\i.north);
}
\node[work] at (-6,-2.35) {用户角色\\场景};
\node[work] at (-6,-3.05) {B/S/C\\边界};
\node[work] at (-4,-2.35) {Wiki\\构建};
\node[work] at (-4,-3.05) {项目/文件\\搜索 API};
\node[work] at (-2,-2.35) {用户/余额\\代理};
\node[work] at (-2,-3.05) {HTTP/TCP\\发布入口};
\node[work] at (0,-2.35) {远程知识库\\添加};
\node[work] at (0,-3.05) {OpenCode\\启停};
\node[work] at (2,-2.35) {KB Session};
\node[work] at (2,-3.05) {MCP 注册\\白名单};
\node[work] at (4,-2.35) {Compose\\部署};
\node[work] at (4,-3.05) {CI\\回归};
\node[work] at (6,-2.35) {联调缺陷};
\node[work] at (6,-3.05) {验收文档};
