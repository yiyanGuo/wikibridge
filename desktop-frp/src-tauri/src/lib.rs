...
fn create_project(
    ...
    let folder = normalize_project_folder(&input.folder_path)?;
    fs::create_dir_all(&folder).map_err(|error| format!("无法创建项目文件夹: {error}"))?;
    let raw_dir = folder.join("raw");
...
fn project_tree(project: &KnowledgeProject) -> Result<ProjectTreeNodeDto, String> {
    let project_root = project_root_path(project)?;
    build_tree_node(&project_root, &project_root)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .expect("failed to run BearFrps desktop app");
}
...
fn client_from_runtime(runtime: &State<'_, SharedRuntime>) -> Result<ApiClient, String> {
    ...
    Ok(client_from_persisted(&persisted))
}

fn client_from_persisted(persisted: &PersistedState) -> ApiClient {
    ApiClient::from_state(persisted)
}
