...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=llm_wiki)** 는 OpenAI 호환의 풀모달 AI 추론 플랫폼입니다. LLM Wiki는 OpenAI 호환 채팅 엔드포인트를 통해 문서를 수집하므로, Atlas Cloud를 드롭인 백엔드로 연결하여 DeepSeek, Qwen, GLM, Kimi, MiniMax, Claude, GPT, Gemini를 단일 API로 사용할 수 있습니다(벤더별 개별 설정 불필요). **설정 → LLM 제공자** 에서 **Atlas Cloud** 프리셋을 선택하세요(base URL 직접 입력도 가능):

```env
# 설정 → LLM 제공자 → Atlas Cloud (custom, OpenAI 호환)
Base URL: https://api.atlascloud.ai/v1
API Key:  <your-atlascloud-api-key>
Model:    deepseek-ai/deepseek-v4-pro
```

`deepseek-ai/deepseek-v4-pro` 는 추론(reasoning) 모델이므로 컨텍스트 윈도우를 넉넉히 설정하세요(프리셋 기본값 128K). 합리적인 구독: [coding plan](https://www.atlascloud.ai/console/coding-plan).

<details>
<summary>Atlas Cloud의 모든 채팅 모델 (59개)</summary>
