...
**Louvain 社区检测：**
- 基于 graphology-communities-louvain 自动发现知识聚类
- 类型 / 社区 着色模式一键切换 —— 按页面类型或按发现的主题集群查看
- 每个社区计算 cohesion（内聚度）评分，低内聚社区标警告
- 12 色调色板，社区图例显示核心节点标签和成员数

**图谱洞察面板：**
- 右侧展开式面板，自动发现知识结构中的洞察
- **惊奇连接** —— 跨社区、跨类型、边缘↔核心的意外关联，复合评分排序，可消除
- **知识空白** —— 孤立页面（度 ≤ 1）、稀疏社区（cohesion < 0.15）、桥接节点（连接 3+ 个集群）
- 点击洞察卡片高亮图谱中对应节点和边
- **深度研究集成** —— 知识空白和桥接节点一键触发 Deep Research，LLM 自动生成领域精准的搜索主题
