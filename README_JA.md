...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=llm_wiki)** は、OpenAI 互換のフルモーダル AI 推論プラットフォームです。LLM Wiki は任意の OpenAI 互換チャットエンドポイント経由でドキュメントを取り込むため、Atlas Cloud をドロップイン・バックエンドとして接続し、DeepSeek・Qwen・GLM・Kimi・MiniMax・Claude・GPT・Gemini を単一の API で利用できます（ベンダーごとの個別設定は不要）。**設定 → LLM プロバイダー** で **Atlas Cloud** プリセットを選択してください（base URL を手動で設定することも可能）:

```env
# 設定 → LLM プロバイダー → Atlas Cloud（custom, OpenAI 互換）
Base URL: https://api.atlascloud.ai/v1
API Key:  <your-atlascloud-api-key>
Model:    deepseek-ai/deepseek-v4-pro
```

`deepseek-ai/deepseek-v4-pro` は推論（reasoning）モデルです。コンテキストウィンドウは十分に確保してください（プリセットの既定は 128K）。お得なサブスクリプション: [coding plan](https://www.atlascloud.ai/console/coding-plan)。

<details>
<summary>Atlas Cloud のすべてのチャットモデル（59）</summary>
