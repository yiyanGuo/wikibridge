...
> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=llm_wiki)** is a full-modal, OpenAI-compatible AI inference platform. Because LLM Wiki ingests documents through any OpenAI-compatible chat endpoint, you can plug Atlas Cloud in as a drop-in backend and reach DeepSeek, Qwen, GLM, Kimi, MiniMax, Claude, GPT and Gemini through a single API — no per-vendor setup. Pick the **Atlas Cloud** preset in **Settings → LLM provider** (or set the base URL manually):

```env
# Settings → LLM provider → Atlas Cloud (custom, OpenAI-compatible)
Base URL: https://api.atlascloud.ai/v1
API Key:  <your-atlascloud-api-key>
Model:    deepseek-ai/deepseek-v4-pro
```

`deepseek-ai/deepseek-v4-pro` is a reasoning model — keep the context window generous (the preset defaults to 128K). Budget-friendly subscriptions: [coding plan](https://www.atlascloud.ai/console/coding-plan).
