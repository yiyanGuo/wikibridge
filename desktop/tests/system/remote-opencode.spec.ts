...
test('adds and connects a remote LLM Wiki knowledge base', async ({ page }) => {
  ...
  await expect(page.locator('iframe.opencode-frame')).toHaveAttribute('src', /^http:\/\/127\.0\.0\.1:9010\/mock\/session\/session-/);
});

test('normalizes duplicate remote LLM Wiki links and updates the existing card', async ({ page }) => {
  await prepareSystemTestPage(page, {
    remoteKnowledgeBases: [
      {
        ...
        status: 'ready',
        projectCount: 1,
        projects: [{ id: 'remote-project-1', name: '团队项目', path: '/remote/team', current: true }],
        currentProject: { id: 'remote-project-1', name: '团队项目', path: '/remote/team', current: true },
        authRequired: false,
