    ...
    <ReactMarkdown
      ...
      components={{
        a({ href, children, ...props }) {
          if (href?.startsWith("wiki:")) {
            ...
            return (
              <button
                className="wiki-link"
                onClick={(event) => openResolvedDocument(event, documentId)}
                type="button"
              >
                {children}
              </button>
          ...
          const linkedDocumentId = href
            ? resolveMarkdownDocumentHref(href, currentDocumentId, treeIndex)
            : null
          if (linkedDocumentId) {
            return (
              <button
                className="wiki-link"
                onClick={(event) => openResolvedDocument(event, linkedDocumentId)}
                type="button"
              >
                {children}
              </button>
        ...
        img({ src, alt }) {
          ...
          const rawSrc =
            isObsidianImage && src ? safeDecode(src.slice("obsidian-image:".length)) : src || ""
...
function applyCalloutMetadata(blockquote: Record<string, unknown>) {
  ...
  const firstText = firstParagraph.children.find(
    (child) => isAstNode(child) && child.type === "text" && typeof child.value === "string",
  )
...
function shouldSkipAstNode(node: Record<string, unknown>) {
  return ["code", "inlineCode", "link", "linkReference", "image", "imageReference"].includes(
    String(node.type),
  )
}
...
function resolveMarkdownDocumentHref(
  href: string,
  currentDocumentId: string,
  treeIndex: TreeIndex,
) {
...
function resolveDocumentTarget(target: string, currentDocumentId: string, treeIndex: TreeIndex) {
  ...
  const byPath = treeIndex.documentNodes.find(
    (node) =>
      stripMarkdownExtension(
        normalizeRelativePath(node.document_id || node.node_id),
  ...
  const byBaseName = treeIndex.documentNodes.find(
    (node) =>
      stripMarkdownExtension(baseName(node.document_id || node.node_id)).toLowerCase() ===
      targetBase,
  )
...
function resolveFileTarget(
  ...
  const candidates = [
    joinRelative(currentDir, cleanTarget),
