...
export default function App() {
  ...
  async function linkProject(project: KnowledgeProject) {
    ...
    setError('');
    try {
      const graph = await invoke<WikiGraphResult>('refresh_wiki_graph', { projectId: project.project_id });
      setWikiGraph(graph);
          ...
          <section className="panel reader-panel">
            ...
            <div className="project-toolbar">
              ...
              <div className="card-actions reader-actions">
                ...
                <button className="secondary" onClick={() => importWikiSources(readerProject, 'folder')} disabled={busy === `add-folder-${readerProject.project_id}`}>
                  <FolderOpen size={17} />
                  Add 文件夹
                </button>
              ...
              <article className="reader-content">
                ...
                {readableFileCount === 0 ? (
                  <div className="empty-state">
                    {(wikiProjectState?.sourceCount ?? 0) > 0 ? '暂无 Wiki 文档，构建后刷新。' : '请先 Add source，再构建。'}
                  </div>
                ) : readerContent ? (
                  <>
          ...
          <section className="panel">
            ...
            <form className="project-form" onSubmit={createProject}>
              ...
              <div className="folder-input">
                <input value={projectFolder} readOnly aria-label="项目保存位置" placeholder="选择保存位置" />
