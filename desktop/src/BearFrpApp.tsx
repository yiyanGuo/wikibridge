...
export default function App() {
  ...
  const connectableProjects = projects.filter(
    (project) => !connectedProjectIds.has(project.project_id),
  )
  ...
  const updateConnectableProject = useCallback(
    (nextProjects: KnowledgeProject[], nextConnections: ProjectConnection[]) => {
  ...
  const refreshAuthenticatedWorkspace = useCallback(async () => {
    const [nextProjects, nextConnections] = await Promise.all([
      refreshProjects(),
      refreshConnections(),
    ])
  ...
  async function importWikiSources(project: KnowledgeProject, mode: "files" | "folder") {
    const selected = await open({ multiple: mode === "files", directory: mode === "folder" })
    ...
    setBusy(`${mode === "files" ? "add" : "add-folder"}-${project.project_id}`)
    setError("")
    try {
      const result = await invoke<WikiImportResult>("import_wiki_sources", {
        input: { projectId: project.project_id, paths: filePaths },
      })
      setWikiProjectState((current) =>
        current
          ...
          : {
              ...
              queue: result.queue,
              failedTasks: [],
              sourceCount: result.importedPaths.length,
              wikiCount: 0,
            },
      )
      ...
      const skipped = result.skippedPaths.length ? `，跳过 ${result.skippedPaths.length} 个` : ""
      setNotice(`已导入 ${result.importedPaths.length} 个 source${skipped}`)
    } catch (err) {
      setError(friendlyError(err))
    } finally {
      setBusy("")
  ...
  async function buildProject(project: KnowledgeProject) {
    ...
    try {
      ...
      if (result.failedCount > 0) {
        ...
        setNotice(
          `构建完成：处理 ${result.processedCount} 个 source，生成 ${result.writtenPaths.length} 个 Wiki 文件`,
        )
      } else {
        setNotice(
  ...
  async function refreshReaderDocuments(projectId: string, preferredDocumentId?: string) {
    ...
    const selectedDocumentId =
      findReadableFile(tree, preferredDocumentId)?.document_id ||
      findReadableFile(tree)?.document_id
  ...
  async function deleteProject(project: KnowledgeProject) {
    if (!window.confirm(`从本地列表移除项目 ${project.name}？\n\n项目文件夹不会从磁盘删除。`))
      return
  ...
  async function startConnection(connection: ProjectConnection) {
    ...
    setError("")
    try {
      const next = await invoke<ProjectConnection>("start_connection", {
  ...
  async function stopConnection(connection: ProjectConnection) {
    ...
    setError("")
    try {
      const next = await invoke<ProjectConnection>("stop_connection", {
        connectionId: connection.connection_id,
      })
      setConnections((current) => upsertConnection(current, next))
      setNotice("访问已关闭")
  ...
  async function deleteConnection(connection: ProjectConnection) {
    ...
    try {
      ...
      setConnections((current) =>
        current.filter((item) => item.connection_id !== connection.connection_id),
      )
      ...
      <header className="topbar">
        ...
        <nav className="main-nav" aria-label="主导航">
          ...
          <button
            ...
            onClick={() => switchPage("connections")}
          >
            <Link2 size={17} />
            访问连接
          </button>
        ...
        <main className="reader-layout">
          <section className="panel reader-panel">
            <div className="reader-header">
              ...
              <div>
                <h1>{wikiProjectState?.project.name || readerProject.name}</h1>
                <p>{wikiProjectState?.project.path || readerProject.folder_path}</p>
              </div>
              <span className="count-pill">{readableFileCount} 篇 Wiki</span>
            </div>
            ...
            <div className="project-toolbar">
              ...
              <div className="card-actions reader-actions">
                <button
                  className="secondary"
                  onClick={() => importWikiSources(readerProject, "files")}
                  disabled={busy === `add-${readerProject.project_id}`}
                >
                  <FilePlus2 size={17} />
                  Add 文件
                </button>
                <button
                  className="secondary"
                  onClick={() => importWikiSources(readerProject, "folder")}
                  disabled={busy === `add-folder-${readerProject.project_id}`}
                >
                  <FolderOpen size={17} />
                  Add 文件夹
                </button>
                <button
                  className="secondary"
                  onClick={() => buildProject(readerProject)}
                ...
                <button
                  ...
                  title="删除项目"
                  onClick={() => deleteProject(readerProject)}
                  disabled={busy === `delete-project-${readerProject.project_id}`}
                >
                  <Trash2 size={17} />
                </button>
              </div>
            ...
            {!llmConfigDraft.configured && (
              <div className="inline-alert reader-config-alert">
                请先配置 DeepSeek API Key，再构建知识库。
              </div>
            )}
              ...
              <aside className="reader-sidebar">
                <div className="reader-sidebar-section">
                  ...
                  {!readerTree || readerTree.children.length === 0 ? (
                    <div className="toc-empty">暂无 Wiki 文档</div>
                  ) : (
                    ...
                    <div className="markdown-body">
                      <ReaderMarkdown
                        ...
                        onOpenDocument={(documentId) =>
                          readDocument(readerProject.project_id, documentId)
                        }
                ...
                <div className="llm-config-grid compact">
                  <label>
                    ...
                    <input
                      ...
                      placeholder="sk-..."
                      onChange={(event) =>
                        setLlmConfigDraft((current) => ({ ...current, apiKey: event.target.value }))
                      }
                    />
                  </label>
                  <button
                    ...
                    <article className="project-card" key={project.project_id}>
                      ...
                      <div className="card-actions">
                        <button
                          ...
                          disabled={busy === `reader-${project.project_id}`}
                        >
                          <BookOpen size={17} />
                          进入项目
                        </button>
                    ...
                    <article className="connection-card" key={connection.connection_id}>
                      <div className="card-heading">
                        ...
                        <span
                          ...
                          data-running={connection.running || undefined}
                        >
...
function findReadableFile(
  ...
  if (
    node.kind === "file" &&
    node.readable &&
    (!preferredDocumentId || node.document_id === preferredDocumentId)
  )
    return node
...
function friendlyError(error: unknown) {
  ...
  if (
    ...
    text.includes("Graph") ||
    text.includes("ingest queue")
  )
    return text
  if (text.includes("文档") || text.includes("Markdown")) return text
  if (text.includes("可用额度") || text.includes("余额不足") || text.includes("流量额度"))
    return text
