...
pub fn ensure_opencode_stack_running(
    ...
    let opencode_port = ensure_opencode_running(app, runtime, llm_wiki_port)?;
...
fn ensure_opencode_running(
    ...
    if let Some(port) = runtime.opencode_stack.opencode_port {
        ...
        runtime.opencode_stack.opencode = None;
        runtime.opencode_stack.opencode_port = None;
    }

    let port = allocate_port(OPENCODE_START_PORT, OPENCODE_END_PORT)?;
    ...
    ensure_executable(&binary);
    let log_path = runtime.paths.log_dir.join("opencode.log");
    let process = spawn_logged(
        binary,
        &[
            "serve".to_string(),
...
fn find_sidecar_binary(app: &AppHandle, group: &str, name: &str) -> Result<PathBuf, String> {
    ...
    Err(format!(
        "未找到 {name} 二进制，请放置到 desktop/src-tauri/binaries/{group}/{platform}/{executable}"
    ))
}
